module YouTubeModel # :nodoc:
  ...
  module SingletonMethods
    ...
    # Find favorite videos by a user							ADDED
    def favorites_of(username)
    	request("users/#{username}/favorites?v=2")
  	end
  	
  	# Find videos uploaded by a certain developer       ADDED
  	def developed_by_me(options = {})
      options[:orderby] ||= :published
      options.delete(:alt)
      params = Hash[*options.stringify_keys.collect{ |k, v|
          [k.dasherize, v] }.flatten
      ]  
  	  request("videos/-/%7Bhttp%3A%2F%2Fgdata.youtube.com%2Fschemas%2F2007%2Fdevelopertags.cat%7D#{YT_CONFIG['developer_tag']}#{query_string(params)}")
	  end
	  
	  #find videos uploaded by a certain kuiq user      ADDED
	  def uploaded_to_kuiq_by(username, options = {})
	    options[:orderby] ||= :published
      options.delete(:alt)
      params = Hash[*options.stringify_keys.collect{ |k, v|
      [k.dasherize, v] }.flatten
      ]  
  	  request("videos/-/%7Bhttp%3A%2F%2Fgdata.youtube.com%2Fschemas%2F2007%2Fdevelopertags.cat%7D#{YT_CONFIG['developer_tag']}_#{username}#{query_string(params)}")
	  end
    ...
    #check to see if video exists on YouTube        ADDED
    def exists?(id)
      response = connection.head("http://gdata.youtube.com/feeds/api/videos/#{id}")
    ...
    #
    # changed to check id is well-formed and then check if the corresponding video exists       EDITED
    def find_by_id(id)
      id =~ /^[a-zA-Z0-9_-]{11}$/ && exists?(id) ? request("videos/#{id}") : request("videos/#{YT_CONFIG['default_yt_id']}")
    end
    ...
    def request(url)
      ...
      extend_attributes(connection.get(url, headers))
    end
    ...
    def build_xml_entry(attrs)
      ...
      xml.entry :xmlns => 'http://www.w3.org/2005/Atom',
        ...
        xml.media :group do
          ...
          xml.media :category, "#{YT_CONFIG['developer_tag']}", :scheme => 'http://gdata.youtube.com/schemas/2007/developertags.cat'
          xml.media :category, "#{YT_CONFIG['developer_tag']}_#{attrs[:kuiq_user]}", :scheme => 'http://gdata.youtube.com/schemas/2007/developertags.cat'
